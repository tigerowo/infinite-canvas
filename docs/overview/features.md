...
- 画布云端同步不会保存浏览器临时 `blob:` 地址；跨域名/反代打开时依赖稳定的 `https` 媒体链或已启用的对象存储 `server:` 文件
- 打开画布时会自动修复媒体：稳定 `https`/`server:` 直接可用；本机 IndexedDB 仍有 `image:`/`file:` 且对象存储可用时自动上传回写；仅剩外站 `blob:` 的历史节点标记失效并提示重新生成
- 换浏览器/反代打开时：优先 `server:`（WebDAV）与稳定 `https`；若节点仍带 `imageTaskId`/`videoTaskId`/`audioTaskId` 且任务记录仍有结果 URL，会自动回填。**不会**扫描 WebDAV 目录做文件比对猜图；Grok 等远程 https 不强制再存一份 WebDAV
