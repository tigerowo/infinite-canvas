...
| 无限画布 ChatGPT 订阅文本节点 | 真实 CLI 验证通过 | 选择 `gpt-5.5 · CLI` 后，普通文本节点经受控订阅代理 completion 返回有效文本；修复前的“指定渠道不可用”来自误入普通 API Provider 校验，修复后用户确认完整链路跑通，且没有回退到付费 API。 |
| 生图工作台 | 通过 | `/image` 正常渲染模型、接口模式、参考图、质量、尺寸和生成数量。 |
| 生图工作台 Nano Banana 2 | 真实 CLI 验证通过 | 用户逐次确认后，`nano-banana-2` 通过 Antigravity Google 登录态和内置 `generate_image` 成功返回 1 张图片，耗时约 1 分 16 秒；结果卡明确显示渠道与调用路径，没有回退到其他渠道或付费 API。 |
...
| `vitest run` | 通过；本轮完整执行 14 个测试文件、49 个测试，覆盖连接中心、受控 CLI、工作台路由与 Nano Banana 2 生成契约。 |
| `playwright test` | 通过；10 个 Chromium Mock 浏览器用例全部通过，总耗时约 1.6 分钟；覆盖编辑、模型拉取、复制不继承密钥、删除确认，以及连接中心模型进入生图台、视频台和无限画布。 |
...
- Nano Banana 2 首次真实闭环曾被 Google 以 `User location is not supported for the API use` 拒绝；切换当前 Mac 到可用代理出口并重新取得用户确认后生成成功。该结果只证明本机当前网络路径可用，不代表所有代理地区持续可用；系统保留明确失败，不自动更换出口或回退到其他渠道和付费 API。
