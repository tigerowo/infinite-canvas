...
| Gemini 原生 | 使用官方 `generativelanguage.googleapis.com` 端点完成一次连接测试和一次模型拉取，模型目录成功返回；默认候选设为 `gemini-2.5-flash-lite`，最低成本生成请求尚未执行。 |
...
| Gemini 原生 | 已通过 | 已通过 | 未执行，等待单次计费确认 |
...
- Gemini 原生的最低成本真实请求以及通用 HTTP 的连接、模型拉取和最低成本请求仍待验证；已通过的 OpenOx Gemini 图片请求属于 OpenAI 兼容协议。准备凭据时应写入被 Git 忽略的本地配置或系统钥匙串，不应粘贴到对话中。
...
- 编辑 Gemini 原生连接并保存新的默认模型后，现有更新接口会把刚通过的连接状态重置为 `untested`。报告仍按实际调用记录连接和模型拉取已通过；本轮没有为了恢复 UI 标签重复测试，后续应让不影响连接的字段更新保留最近测试状态。
