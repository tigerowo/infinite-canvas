...
阶段开始时本机没有已配置 Provider、旧 `localChannels` 或 provider 类环境变量。后续用户在连接中心安全配置 RunningHub 后，完成一次账户检测和一次真实工作流任务；任务只提交一次，状态为 `QUEUED → RUNNING → SUCCESS`，返回 1 个 PNG。没有读取未知钥匙串项，也没有在终端、浏览器、报告或 Git 中输出密钥。
...
| OpenAI 兼容 | `gpt-image-2` 完成一次连接测试、模型拉取和最低成本生图，连接中心到生图台的完整链路成功。OpenOx 的 `gemini-3.1-flash-image-preview` 返回了 Markdown 包裹的图片 Data URL，暴露并修复了解析兼容问题；重启 Go 后端后的 UI 成功态尚未复验。 |

### 因缺少凭据未验证

| 协议 | 连接测试 | 模型拉取 | 最低成本请求 |
| --- | --- | --- | --- |
| OpenAI 兼容 | 已通过 | 已通过 | `gpt-image-2` 已通过；OpenOx Gemini 图片解析修复待重启复验 |
...
- Gemini 原生和通用 HTTP 的最低成本真实请求仍待验证。准备凭据时应写入被 Git 忽略的本地配置或系统钥匙串，不应粘贴到对话中。
