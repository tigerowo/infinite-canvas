...
- Mac CLI helper 的版本探测已移入独立 Unix Socket 伴随进程，并增加逐次 HMAC/nonce 防重放授权、签名响应、Ed25519 清单、过期时间和二进制 SHA-256 强校验。Codex 支持独立只读登录状态动作，以及用户二次确认后的固定浏览器 OAuth：登录进程全局互斥、最长 10 分钟且不返回任何命令输出；Gemini/即梦没有稳定非交互状态契约，因此不猜测执行。安装与真实模型调用仍未实现或执行，本轮没有实际触发登录。
