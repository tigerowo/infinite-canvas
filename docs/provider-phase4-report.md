...
- Mac CLI helper 的版本探测已移入独立 Unix Socket 伴随进程，并增加逐次 HMAC/nonce 防重放授权、签名响应、Ed25519 清单、过期时间和二进制 SHA-256 强校验。Codex 新增独立只读登录状态动作，固定执行官方 `codex login status` 且不返回账号信息；Gemini/即梦没有稳定非交互状态契约，因此不猜测执行。安装、交互登录与真实模型调用仍未实现或执行。
