...
- 使用正式 Apple Developer ID 和公证配置构建、安装并验收 Mac CLI helper 发布包；个人自用的隔离开发安装模式已完成安装、登录状态检查及一次端到端 Codex CLI 最小真实调用。本轮没有可用签名身份，因此仍未生成或验收正式发布包。Gemini/即梦因缺少稳定非交互状态命令暂不开放。
