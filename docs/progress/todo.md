...
- 逐步清理前端 ESLint 兼容基线警告，并继续扩展浏览器端测试；当前已有正式 lint、typecheck、Vitest 脚本、任务取消服务契约测试、连接中心 React 组件状态测试，以及覆盖基础状态、API 抽屉、迁移预览和生图/视频模型来源的 Playwright Mock 回归，后续再覆盖连接测试、编辑/复制/删除和画布模型选择链路。
