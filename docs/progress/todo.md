...
- 为受控 Mac CLI helper 设计安装动作和分发流程；独立 Unix Socket、逐次授权、签名清单、固定版本检测、Codex 只读登录状态、浏览器 OAuth 及逐次确认的最小模型调用已接入，本机签名清单安装、CLI 检测、登录状态和调用确认窗已人工验收。尚未实现正式安装器与 helper 签名发布，也未实际触发 OAuth 或模型调用；伴随进程未启动、Socket 权限错误和无效清单仍需人工回归。Gemini/即梦因缺少稳定非交互状态命令暂不开放。
