...
| `POST` | `/providers/:id/cli/detect` | 仅本机回环请求可触发受控 CLI 检测；Antigravity 同时只读拉取模型，Gemini 官方 CLI 载入固定官方别名 |
...
| `POST` | `/providers/:id/cli/model-probe` | 仅本机回环请求且请求体 `confirmed=true` 时可启动一次受控 CLI 或实验本机订阅代理固定最小调用 |
| `POST` | `/providers/:id/cli/completions` | 仅本机回环请求可为无限画布启动一次受控 CLI 或实验本机订阅代理文本任务 |
| `POST` | `/providers/:id/cli/generations` | 逐次确认后启动即梦、GPT Image 2 订阅 helper、Codex 应急生成或实验本机订阅代理图片任务 |
...
CLI Provider 的检测目录与工作台目录分开保存：`models` 保留检测到的完整模型列表，`verified_models` 只记录真实任务状态为 `succeeded` 的模型。明确 `failed` 会移除对应成功记录，`cancelled` 和 `timed_out` 不改变既有记录。画布与工作台只投影 `verified_models`；原 Provider 通道身份仍会保留，以阻止失效模型自动切换到其他同名渠道或付费 API。
...
helper 不安装 CLI、不读取 shell profile，也不接受任意命令。伴随进程允许固定 `--version` 检测、Codex `login status`、用户确认后的固定 `codex login`，以及受控模型调用；Antigravity 额外允许只读 `agy models`，其成功结果同时作为非交互登录状态并保存最多 64 个受校验模型名。Gemini 官方 CLI 没有接入未确认的登录状态或模型列表命令，检测只载入 `flash-lite`、`flash`、`pro`、`auto` 四个官方别名，真实会话必须由用户确认的最小调用验证。可执行文件必须位于受控目录，解析后的普通文件不得允许组或其他用户写入，文件大小不得超过 256 MiB，并且 SHA-256 必须与签名清单一致。工作目录字段只作为元数据保存，模型调用始终进入新建私有临时目录。
...
Codex 最小调用和画布文本固定执行 `codex exec --model gpt-5.5 --sandbox read-only --skip-git-repo-check --ephemeral --output-last-message <临时文件> ...`；画布提示词只通过 stdin 传入，不会被解释成参数。Antigravity 文本任务固定使用 `agy --print <提示词> --output-format text --model <已拉取模型> --effort low --print-timeout 90s --disable-slash-commands --mode plan --sandbox`。Gemini 官方 CLI 文本任务固定使用 `gemini --prompt "" --output-format json --model <受控别名> --approval-mode plan --sandbox --skip-trust`，提示词同样只通过 stdin 传入，最终只读取 JSON 的 `response` 字段并限制为 4 KiB。
...
两条路径使用不同 Provider、模型 ID 和任务前缀，便于本地日志分开核对；这只是本地调用边界，不代表 ChatGPT 服务端提供两个独立额度池。两者都只接受文生图、固定比例和质量档位，最多保留一张不超过 32 MiB 的 PNG。成功文件由 Go 后端校验 PNG 头、受控临时目录和大小后转存当前对象存储，再向工作台返回素材 URL。全局只允许一个活动 CLI 调用，图片 deadline 为 3 分钟，取消会终止本机子进程。

### 实验性本机订阅代理

连接中心另提供两个默认关闭的实验 CLI 协议，用于把仓库外独立运行的 CLIProxyAPI 接入既有受控 helper：

- `chatgpt-subscription-proxy`：只开放 `gpt-5.5` 文本与 `gpt-image-2` 图片。
- `antigravity-subscription-proxy`：只开放 `gemini-3.1-flash-lite` 文本与 `gemini-3.1-flash-image` 图片。

这两个协议不接受页面填写的 Base URL、API Key、命令或额外模型。helper 仅连接 `http://127.0.0.1:8317`，禁用系统代理并拒绝重定向；访问密钥只从仓库外权限为 `0600` 的文件读取。模型检测读取 `/v1/models` 后只把严格白名单交集放入可测试目录，明确的模型不可用失败会撤销对应记录。文本与图片继续使用既有异步任务状态、全局单任务、deadline、取消传播和错误脱敏；图片只接受 Base64/Data URL，不继续下载上游给出的远端 URL。

连接测试不等于订阅能力已验证。文本必须由用户确认一次固定最小调用，图片必须在工作台显示渠道、模型和任务类型后逐次确认。任一路径失败都不会自动重试、切换现有 Codex/Antigravity 渠道或回退到按次计费 API。外部 CLIProxyAPI 的安装、OAuth、版本和进程生命周期不由连接中心管理，完整开发配置见 [Mac CLI helper 安装与签名发布](cli-helper-macos.md)。
...
共享密钥属于本机进程凭据，只能保存在权限为 `0600` 的共享密钥文件、被忽略的本地 `.env`、受保护的启动环境或系统钥匙串中，不得写入文档、日志或 Git。安装只通过用户显式运行的本机脚本完成，Web UI 和 helper API 不开放安装动作。页面不接收 API Key、Access Token 或 OAuth 授权码；Codex、GPT Image 2、Antigravity 和 Gemini 官方 CLI 登录材料均由各自 CLI 在本机管理。
