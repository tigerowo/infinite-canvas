...
describe("provider helpers", () => {
    ...
    it("adds connected text, subscription image, emergency image, Antigravity and Jimeng models to the shared workbench catalog", () => {
        const providers = [
            ...
            { id: "provider-gemini-cli", kind: "cli", protocol: "gemini-cli", name: "Antigravity CLI", capabilities: ["text", "image"], models: ["gemini-3.5-flash-low", "nano-banana-2"], defaultModel: "gemini-3.5-flash-low", connectionStatus: "connected", enabled: true },
        ...
        expect(providerModelChannels(providers)).toEqual([
            ...
            expect.objectContaining({ id: "provider-gemini-cli", protocol: "gemini-cli", models: ["gemini-3.5-flash-low", "nano-banana-2"], capabilities: ["text", "image"] }),
