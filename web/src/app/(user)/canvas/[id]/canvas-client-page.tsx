...
import { assistantDraftInputs, canvasDraftInputs } from "@/extensions/media-lifecycle/canvas-drafts";
import { protectSubmittedReferences } from "@/extensions/media-lifecycle/use-material-draft";
...
import { applyMediaDimensions, acceptsVideoTaskUpdate } from "@/extensions/media-reliability/canvas-media";
import { isTaskLookupRetry, taskLookupMessage } from "@/extensions/media-reliability/task-state";
import { mergeProjectChanges } from "@/extensions/media-reliability/project-merge";
import { useCanvasArchive } from "@/extensions/media-reliability/canvas-archive";
...
import { buildNodeChatMessages, buildNodeGenerationContext, buildNodeGenerationInputs, canvasReferenceLimitError, hydrateNodeGenerationContext, type NodeGenerationContext, type NodeGenerationInput } from "../components/canvas-node-generation";
...
function InfiniteCanvasPage({ projectId }: { projectId: string }) {
    ...
    const remoteRevision = useCanvasStore((state) => state.remoteRevision);
    ...
    const liveProjectStateRef = useRef({ nodes, connections, chatSessions, activeChatId, agentConfig, backgroundMode, showImageInfo });
    liveProjectStateRef.current = { nodes, connections, chatSessions, activeChatId, agentConfig, backgroundMode, showImageInfo };
    useCanvasArchive(nodes, setNodes, message.warning);
    ...
    useEffect(() => {
        ...
        let cancelled = false;
        const before = liveProjectStateRef.current;
        const restore = async () => {
            const initialLoad = loadedProjectIdRef.current !== projectId;
            const restoredNodes = await hydrateCanvasImages(initialLoad ? resetInterruptedGeneration(project.nodes) : project.nodes);
            if (cancelled) return;
            const restoredSessions = syncAssistantReferences(project.chatSessions || [], restoredNodes, initialLoad);
            const restored = { nodes: restoredNodes, connections: project.connections, chatSessions: restoredSessions, activeChatId: project.activeChatId || null, agentConfig: project.agentConfig || null, backgroundMode: project.backgroundMode, showImageInfo: project.showImageInfo || false };
            const next = initialLoad ? restored : mergeProjectChanges(before, liveProjectStateRef.current, restored);
            const current = liveProjectStateRef.current;
            const unchanged = !initialLoad && JSON.stringify(current) === JSON.stringify(next);
            if (!unchanged) {
                setNodes(next.nodes);
                setConnections(next.connections);
                setChatSessions(next.chatSessions);
                setActiveChatId(next.activeChatId);
                setAgentConfig(next.agentConfig);
            ...
            if (initialLoad) {
                ...
                void touchLifecycleEntity("canvas", projectId, "open").catch(() => undefined);
            }
    ...
    }, [hydrated, openProject, projectId, remoteRevision, router]);
    ...
    useEffect(() => {
        if (!projectLoaded || restoringProjectRef.current || historyPausedRef.current) return;
    ...
    useEffect(() => {
        ...
        const pollCanvasTasks = () => {
            const videoTargets = nodesRef.current.filter((node) => node.type === CanvasNodeType.Video && (node.metadata?.status === NODE_STATUS_LOADING || (node.metadata?.status === NODE_STATUS_ERROR && isTaskLookupRetry(node.metadata.errorDetails))) && !node.metadata.content && canvasVideoTaskId(node.metadata));
            videoTargets.forEach((node) => {
                ...
                void pollVideoGenerationTaskStatus(generationConfig, canvasVideoTaskFromMetadata(node.metadata))
                    ...
                    .catch((error: unknown) => {
                        const errorDetails = taskLookupMessage(error);
                        setNodes((prev) => prev.map((item) => item.id === node.id && canvasVideoTaskId(item.metadata) === taskId && item.metadata?.startedAt === node.metadata?.startedAt && (item.metadata?.status === NODE_STATUS_LOADING || isTaskLookupRetry(item.metadata?.errorDetails))
            ...
            const imageTargets = nodesRef.current.filter((node) => isCanvasImageNodeType(node.type) && (node.metadata?.status === NODE_STATUS_LOADING || isTaskLookupRetry(node.metadata?.errorDetails)) && !node.metadata?.content && node.metadata?.imageTaskId);
            imageTargets.forEach((node) => {
                ...
                void pollCanvasImageTaskStatus(node.metadata.imageTaskId)
                    ...
                    .catch((error: unknown) => {
                        setNodes((prev) => prev.map((item) => item.id === node.id && item.metadata?.imageTaskId === node.metadata?.imageTaskId && item.metadata?.startedAt === node.metadata?.startedAt && (item.metadata?.status === NODE_STATUS_LOADING || isTaskLookupRetry(item.metadata?.errorDetails))
                            ? { ...item, metadata: { ...item.metadata, status: NODE_STATUS_ERROR, errorDetails: taskLookupMessage(error) } } : item));
                    })
                    .finally(() => {
    ...
    const visibleNodes = useMemo(() => {
        ...
        // A screen-space editor must survive culling of its offscreen anchor node.
        return nodes.filter((node) => !isHiddenBatchChild(node, nodes, collapsingBatchIds) && (node.id === dialogNodeId || (node.position.x + node.width > viewLeft && node.position.x < viewRight && node.position.y + node.height > viewTop && node.position.y < viewBottom)));
    }, [collapsingBatchIds, dialogNodeId, nodes, size.height, size.width, viewport.k, viewport.x, viewport.y]);
    ...
    const createAndOpenProject = useCallback(() => {
        const id = createProject(`微鑫画布 ${useCanvasStore.getState().projects.length + 1}`);
    ...
    const handleNodeMouseDown = useCallback((event: ReactPointerEvent, nodeId: string) => {
        ...
        dragRef.current = {
            ...
            initialPositionsById: new Map(currentNodes.filter((node) => dragIds.has(node.id)).map((node) => [node.id, { x: node.position.x, y: node.position.y }])),
        };
    ...
    const handleGlobalMouseMove = useCallback(
        (event: PointerEvent) => {
            ...
            if (dragRef.current.isDraggingNode) {
                ...
                const initialPositions = dragRef.current.initialSelectedNodes;
                const initialPositionsById = dragRef.current.initialPositionsById;
                ...
                const previewNodes = nodesRef.current.map((node) => {
                    const initial = initialPositionsById.get(node.id);
                    return initial ? { ...node, position: { x: initial.x + dx, y: initial.y + dy } } : node;
                });
    ...
    const handleMediaLoaded = useCallback((nodeId: string, source: string, width: number, height: number) => {
        setNodes((previous) => {
            const node = previous.find((item) => item.id === nodeId);
            if (!node) return previous;
            const next = applyMediaDimensions(node, source, width, height);
            return next === node ? previous : previous.map((item) => item.id === nodeId ? next : item);
        });
    }, []);
    ...
    const downloadNodeImage = useCallback((node: CanvasNodeData) => {
        ...
        void touchLifecycleEntity("canvas", projectId, "download").catch(() => undefined);
    ...
    const uploadNodeMediaToCloud = useCallback(async (node: CanvasNodeData) => {
        if ((node.type !== CanvasNodeType.Video && node.type !== CanvasNodeType.Audio) || !node.metadata?.content || node.metadata.archivePending || node.metadata.storageKey?.startsWith("server:") || uploadingMediaNodeIdsRef.current.has(node.id)) return;
        ...
        try {
            ...
            const uploaded = await uploadRemoteMediaToServer(mediaUrl, filename, true);
    ...
    const uploadNodeImageToCloud = useCallback(async (node: CanvasNodeData) => {
        if (!isCanvasImageNodeType(node.type) || !node.metadata?.content || node.metadata.archivePending || node.metadata.storageKey?.startsWith("server:") || uploadingImageNodeIdsRef.current.has(node.id)) return;
        ...
        try {
            ...
            const uploaded = await uploadRemoteImageToServer(imageUrl, "canvas-image-" + node.id + ".png", true);
            setNodes((nodes) => nodes.map((item) => (item.id === node.id ? {
                ...item,
                metadata: {
                    ...item.metadata,
    ...
    const handleGenerateNode = useCallback(
        async (nodeId: string, mode: CanvasNodeGenerationMode, prompt: string, onAccepted?: () => void, externalInputs?: NodeGenerationInput[]) => {
            if (submittingNodes.current.has(nodeId)) { message.info("该节点正在提交，请稍候"); return; }
            submittingNodes.current.add(nodeId);
            try {
            const sourceNode = nodesRef.current.find((node) => node.id === nodeId);
            const generationConfig = buildGenerationConfig(effectiveConfig, sourceNode, mode);
            if (!isAiConfigReady(generationConfig, generationConfig.model)) {
                if (!generationConfig.model.trim()) message.warning(`当前没有可用的${mode === "video" ? "视频" : mode === "image" ? "图片" : mode === "audio" ? "音频" : "文本"}模型，请先配置模型`);
                openConfigDialog(true);
                return;
            }
            ...
            const draftLocation = `canvas:${projectId}:${sourceNode?.type === CanvasNodeType.Config ? "config" : "node"}:${nodeId}`;
            const inputContext = buildNodeGenerationContext(nodeId, nodesRef.current, connectionsRef.current, editingTextNode ? `请根据要求修改以下文本。\n\n原文：\n${sourceTextContent}\n\n修改要求：\n${prompt}` : prompt, externalInputs || await canvasDraftInputs(draftLocation));
            const limitError = canvasReferenceLimitError(inputContext);
            if (limitError) {
                message.warning(limitError);
                return;
            }
            await protectSubmittedReferences([...inputContext.referenceImages, ...inputContext.referenceVideos, ...inputContext.referenceAudios, ...inputContext.videoElementList.flatMap((item) => item.references || [])], draftLocation);
            ...
            try {
                ...
                if (mode === "video") {
                    ...
                    const clientTaskId = `client_video_task_${videoId}_${nanoid()}`;
                    ...
                    const created = await createVideoGenerationTask(videoGenerationConfig, requestPrompt, { references: videoReferenceImages, videoReferences: generationContext.referenceVideos, audioReferences: generationContext.referenceAudios }, undefined, { clientTaskId, source: "canvas", sourceId: videoId });
                ...
                message.error(error instanceof Error ? error.message : "参考素材准备失败，请稍后重试");
                setRunningNodeId(null);
            } finally { submittingNodes.current.delete(nodeId); }
        },
        [effectiveConfig, message, openConfigDialog, projectId],
    );
    ...
    const executeCanvasAgentAction = useCallback(
        async (action: CanvasAgentAction, messageReferenceNodeIds: string[], externalReferences: CanvasAssistantReference[] = []): Promise<CanvasAgentToolResult> => {
            ...
            try {
                ...
                if (action.name === "generate_image" || action.name === "edit_image" || action.name === "generate_video" || action.name === "generate_audio") {
                    ...
                    const selectedIds = Object.prototype.hasOwnProperty.call(args, "sourceNodeIds")
                        ? stringValues("sourceNodeIds")
                        : messageReferenceNodeIds;
                    const externalIds = new Set(externalReferences.map((reference) => reference.id));
                    const sourceNodeIds = selectedIds.filter((id) => !externalIds.has(id));
                    const externalInputs = assistantDraftInputs(externalReferences.filter((reference) => !Object.prototype.hasOwnProperty.call(args, "sourceNodeIds") || selectedIds.includes(reference.id)));
                    const missingNodeId = validateNodeIds(sourceNodeIds);
                    if (missingNodeId) return missingNodeResult(missingNodeId);
                    const sourceNodes = sourceNodeIds.map((nodeId) => getNode(nodeId)!);
                    if (action.name === "edit_image" && !externalInputs.some((input) => input.image) && !sourceNodes.some((node) => isCanvasImageNodeType(node.type) && Boolean(node.metadata?.content))) {
                    ...
                    await handleGenerateNode(node.id, mode, prompt, undefined, externalInputs);
    ...
    const handleRetryNode = useCallback(
        async (node: CanvasNodeData) => {
            const archiveTaskId = node.metadata?.imageTaskId || node.metadata?.videoTaskId || node.metadata?.audioTaskId;
            try {
                const revision = await retryTaskArchive(archiveTaskId ? { id: archiveTaskId } : undefined, node.type === CanvasNodeType.Video ? "video-task" : node.type === CanvasNodeType.Audio ? "audio-task" : "image-task", node.metadata?.errorDetails);
                if (revision) {
                    setNodes((items) => items.map((item) => item.id === node.id ? { ...item, metadata: { ...item.metadata, archiveRecovery: revision, status: NODE_STATUS_LOADING, progress: 99, errorDetails: undefined } } : item));
            ...
            const retryVideoTaskId = node.type === CanvasNodeType.Video ? `client_video_task_${node.id}_${nanoid()}` : "";
                ...
                >
                    ...
                    {visibleNodes.map((node) => (
                        <CanvasNode
                            ...
                            renderPanel={(panelNode) =>
                                panelNode.type === CanvasNodeType.Config ? (
                                    <CanvasConfigComposer
                                        location={`canvas:${projectId}:config:${panelNode.id}`}
                                    ...
                                    <CanvasNodePromptPanel
                                        location={`canvas:${projectId}:node:${panelNode.id}`}
                                        node={panelNode}
                            ...
                            onMediaActivity={() => void touchLifecycleEntity("canvas", projectId, "play").catch(() => undefined)}
                            onSelectReference={selectNodeReference}
                            onClosePanel={() => setDialogNodeId((current) => current === node.id ? null : current)}
...
function FullscreenPreview({ src, alt, isVideo, isPanorama, proxyGeneratedPanorama = false, onDownload, onClose, hasPrev, hasNext, onPrev, onNext }: { src: string; alt: string; isVideo?: boolean; isPanorama?: boolean; proxyGeneratedPanorama?: boolean; onDownload: () => void; onClose: () => void; hasPrev?: boolean; hasNext?: boolean; onPrev?: () => void; onNext?: () => void }) {
    ...
    return (
        <div className="fixed inset-0 z-[2000] flex items-center justify-center bg-black/80 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label={`${alt}预览`} data-canvas-no-zoom={isPanorama ? "" : undefined} onClick={onClose}>
            {hasPrev || hasNext ? (
                <>
                    <button
                        type="button"
                        ...
                        className="absolute left-4 top-1/2 z-[2010] -translate-y-1/2 flex size-11 items-center justify-center rounded-full border border-white/10 bg-white/5 text-white transition-all hover:scale-105 hover:bg-white/10 active:scale-95 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/70 disabled:cursor-not-allowed disabled:opacity-20"
                        aria-label="上一个"
                    >
                    ...
                    <button
                        ...
                        onPointerDown={(e) => e.stopPropagation()}
                        className="absolute right-4 top-1/2 z-[2010] -translate-y-1/2 flex size-11 items-center justify-center rounded-full border border-white/10 bg-white/5 text-white transition-all hover:scale-105 hover:bg-white/10 active:scale-95 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/70 disabled:cursor-not-allowed disabled:opacity-20"
                        aria-label="下一个"
                    >
                    ...
                    <div className="pointer-events-none absolute inset-x-0 bottom-0 z-10 bg-gradient-to-t from-black/85 via-black/40 to-transparent px-5 pb-3 pt-24">
                        ...
                        <div className="pointer-events-auto mt-2 flex items-center justify-between">
                            <div className="flex items-center gap-1">
                                ...
                                <div className="group/volume flex min-h-11 items-center">
                                    ...
                                    <input type="range" min={0} max={1} step="0.01" value={videoVolume} aria-label="音量" onChange={(event) => { const volume = Number(event.currentTarget.value); if (!videoRef.current) return; if (volume === 0) videoRef.current.muted = true; else { videoRef.current.volume = volume; videoRef.current.muted = false; } }} onPointerUp={() => videoRef.current?.focus({ preventScroll: true })} className="h-1 w-0 pointer-events-none cursor-pointer appearance-none rounded-full opacity-0 transition-[width,opacity] duration-200 focus-visible:pointer-events-auto focus-visible:w-20 focus-visible:opacity-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/70 group-hover/volume:mx-1 group-hover/volume:w-20 group-hover/volume:pointer-events-auto group-hover/volume:opacity-100 [&::-moz-range-thumb]:size-2 [&::-moz-range-thumb]:rounded-full [&::-moz-range-thumb]:border-0 [&::-moz-range-thumb]:bg-white [&::-webkit-slider-thumb]:size-2 [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-white" style={{ background: `linear-gradient(to right, white ${videoVolume * 100}%, rgba(255,255,255,0.35) 0)` }} />
                                </div>
                            </div>
                            <div className="flex items-center gap-1">
...
function CanvasTopBar({
    ...
    return (
        <>
            <div data-canvas-top-bar className="glass-surface pointer-events-none absolute left-0 right-0 top-0 z-50 flex h-16 items-center justify-between border-b px-4">
                <div className="pointer-events-auto flex min-w-0 items-center gap-3">
                    <button type="button" onClick={onToggleSidePanel} className="grid size-10 place-items-center rounded-xl transition hover:bg-black/5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-stone-500/50 dark:hover:bg-white/10" style={{ color: theme.node.text }} aria-label={sidePanelOpen ? "收起左侧面板" : "展开左侧面板"}>
                    ...
                    >
                        <button type="button" className="grid size-10 place-items-center rounded-xl transition hover:bg-black/5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-stone-500/50 dark:hover:bg-white/10" style={{ color: theme.node.text }} aria-label="打开画布菜单">
                    ...
                    <div ref={titleRef} className="flex min-w-0 items-center gap-2">
                        {isTitleEditing ? (
                            ...
                            <button
                                type="button"
                                className="max-w-[280px] truncate rounded-md border-b border-dashed border-transparent text-left text-lg font-semibold tracking-normal transition hover:border-current focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-stone-500/50"
                    ...
                    {assistantCollapsed ? (
                        <>
                            ...
                            <Button
                                ...
                                style={{ background: theme.glass.panel, borderColor: theme.glass.border, color: theme.node.text, boxShadow: `${theme.glass.shadow}, inset 0 1px 0 ${theme.glass.highlight}` }}
...
function applyCanvasVideoTaskUpdate(nodes: CanvasNodeData[], nodeId: string, task: VideoResponse, config: AiConfig, startedAt: number, fallbackSize: { width: number; height: number }) {
    return nodes.map((node) => {
        ...
        if (!acceptsVideoTaskUpdate(node, startedAt, completed ? "completed" : task.status)) return node;
...
function applyCanvasImageTaskUpdate(nodes: CanvasNodeData[], nodeId: string, task: CanvasImageTask, startedAt: number, fallbackSize: { width: number; height: number }) {
    ...
    if (!target || !acceptsVideoTaskUpdate(target, startedAt, urls.length ? "completed" : task.status)) return nodes;
...
function canvasAgentVideoDurationHint(modelName: string, config?: AiConfig) {
    if (config && isNewAPIConfig(config)) return { min: 1, max: 15, range: "1-15 秒" };
    ...
    return { values: [6, 10, 12, 15], range: "1-15 秒" };
}

function validateCanvasAgentVideoSeconds(modelName: string, seconds: number, config?: AiConfig) {
    if (config && isNewAPIConfig(config)) return Number.isInteger(seconds) && seconds >= 1 && seconds <= 15 ? "" : "视频时长必须是 1 到 15 之间的整数秒";
    ...
    if (!key.includes("seedance") && !key.includes("kling") && (seconds < 1 || seconds > 15)) return "当前视频模型仅支持 1-15 秒";
    return "";
}
...
function buildGenerationConfig(config: AiConfig, node: CanvasNodeData | undefined, mode: CanvasNodeGenerationMode): AiConfig {
    ...
    return {
        ...config,
        model: resolveModelForCapability(config, node?.metadata?.model || defaultModel || config.model, mode),
        activeChannelId,
        imageChannelId,
        videoChannelId,
        textChannelId,
