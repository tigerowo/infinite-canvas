...
import { ArrowRight, Cable, Coins, Copy, Crown, Ellipsis, History, Import, KeyRound, LogIn, Play, Plus, RefreshCw, Server, Square, TerminalSquare, UserRound } from "lucide-react";
...
export default function ProvidersPage() {
    ...
    const [account, setAccount] = useState<CLIAccountSummary | null>(null);
        ...
        const poll = () => {
            timer = window.setTimeout(() => {
                void queryCLIProbe(token, probeProviderId, probeTaskId)
                    ...
                    .catch((queryError) => setProbe((current) => (current?.result.taskId === probeTaskId ? { ...current, result: { ...current.result, message: queryError instanceof Error ? queryError.message : "调用状态读取失败" } } : current)))
                    .finally(() => {
                        if (active) poll();
                    });
            }, 5000);
    ...
    async function checkLoginStatus() {
        if (!token || !editing || !["codex", "gemini-cli", "jimeng"].includes(editing.protocol)) return;
        ...
        try {
            ...
            if (["codex", "gemini-cli", "jimeng"].includes(checkedItem.protocol)) form.setFieldsValue({ models: checkedItem.models, defaultModel: checkedItem.defaultModel });
    ...
    async function refreshJimengAccount() {
        if (!token || !editing || editing.protocol !== "jimeng") return;
        setLoadingAccount(true);
        try {
            const result = await fetchCLIAccountSummary(token, editing.id);
            if (!result.account) throw new Error(result.message || "即梦账户信息不可用");
            setAccount(result.account);
            message.success(result.message);
        } catch (accountError) {
            setAccount(null);
            message.error(accountError instanceof Error ? accountError.message : "即梦账户信息读取失败");
        } finally {
    ...
    const columns: TableProps<Provider>["columns"] = [
        ...
        { title: "默认模型", dataIndex: "defaultModel", width: "20%", ellipsis: true, render: (value: string) => value || <span className="text-stone-400">未设置</span> },
        {
            ...
            width: "16%",
            render: (value: ProviderStatus, item) => <ProviderStatusTag status={testing && editing?.id === item.id ? "testing" : value} message={testing && editing?.id === item.id ? "正在检测连接，请稍候" : item.statusMessage} />,
        },
                ...
                <div className="mx-auto w-full max-w-7xl px-4 py-6 md:px-8 md:py-9">
                    ...
                    {activeKind === "cli" ? (
                        <Alert
                            ...
                            type="info"
                            showIcon
                            title="受控 Mac CLI helper"
                            description="仅在本机回环地址且显式启用时执行固定动作，不接受用户填写的命令、参数或执行路径。Codex 与 Antigravity 的最小调用、即梦的图片和视频生成都必须在工作台逐次确认。"
                        />
                    ) : null}
                    ...
                    <Form className="[&_.ant-form-item]:mb-4" form={form} layout="vertical" initialValues={initialForm} onFinish={(values) => void submit(values)} requiredMark="optional" scrollToFirstError={{ block: "center" }}>
                        ...
                        <Form.Item
                            ...
                            rules={[
                                ...
                                { max: 80, message: "连接名称不能超过 80 个字符" },
                            ]}
                        >
                        ...
                        {formKind === "api" ? (
                            ...
                            <>
                                <Alert
                                    ...
                                    description={
                                        formProtocol === "codex"
                                            ...
                                            : formProtocol === "gpt-image-2"
                                              ? "默认生图路径。固定调用 gpt-image-2-skill 的 Codex Provider，只读取本机 Codex 登录态；不会读取 OpenAI API Key，也不会自动回退到付费 API。"
                                ...
                                {editing?.kind === "cli" && editing.protocol === "jimeng" && formProtocol === "jimeng" ? (
                                    <section className="mb-5 overflow-hidden rounded-lg border border-stone-200 dark:border-stone-800" aria-label="即梦账户概览">
                                        <div className="flex items-start justify-between gap-3 bg-stone-50/70 px-4 py-3 dark:bg-stone-900/40">
                                            <div>
                                                <div className="font-medium text-stone-950 dark:text-stone-100">即梦账户概览</div>
                                                <div className="mt-1 text-xs leading-5 text-stone-500 dark:text-stone-400">实时读取，不写入数据库；页面不会显示登录凭据。</div>
                                            </div>
                                            <Button size="small" icon={<RefreshCw className="size-3.5" />} loading={loadingAccount} onClick={() => void refreshJimengAccount()}>
                                                刷新
                                            </Button>
                                        </div>
                                        <div className="grid grid-cols-1 divide-y divide-stone-200 sm:grid-cols-3 sm:divide-x sm:divide-y-0 dark:divide-stone-800">
                                            <AccountMetric icon={<UserRound className="size-4" />} label="账号名" value={account ? account.userName || "即梦未返回账号名" : "点击刷新读取"} />
                                            <AccountMetric icon={<Crown className="size-4" />} label="会员等级" value={account?.vipLevel || "—"} />
                                            <AccountMetric icon={<Coins className="size-4" />} label="当前积分" value={account?.totalCredit || "—"} />
                                        </div>
                                        <div className="flex items-start gap-3 border-t border-stone-200 px-4 py-3 dark:border-stone-800">
                                            <History className="mt-0.5 size-4 shrink-0 text-stone-400" />
                                            <div className="min-w-0 flex-1">
                                                <div className="flex items-center gap-2 text-sm font-medium text-stone-700 dark:text-stone-300">
                                                    积分消费明细
                                                    <Tag bordered={false}>后续开放</Tag>
                                                </div>
                                                <div className="mt-1 text-xs leading-5 text-stone-500">已预留展示位置；当前版本不采集、不推算调用消费记录。</div>
                                            </div>
                                        </div>
                                    </section>
                                ) : null}
...
function AccountMetric({ icon, label, value }: { icon: ReactNode; label: string; value: string }) {
    return (
        <div className="min-w-0 px-4 py-3">
            ...
            <div className="mt-1.5 truncate text-sm font-semibold text-stone-950 dark:text-stone-100" title={value}>
                {value}
            </div>
        </div>
    );
        ...
        <article className={`min-w-0 rounded-[12px] border border-black/10 p-4 dark:border-white/[0.06] ${item.enabled ? "bg-white dark:bg-[#1a2122]" : "bg-[#f1f2f3] opacity-70 dark:bg-[#090f10]"}`}>
            ...
            <div className="mt-4 flex min-w-0 items-end justify-between gap-3 border-t border-black/10 pt-3 dark:border-white/[0.06]">
                <div className="min-w-0 max-w-[180px]">
                    ...
                    <div className="mt-1 truncate text-sm font-medium leading-5 text-black/60 dark:text-[rgba(224,245,255,0.60)]" title={item.defaultModel || item.baseUrl || "未设置默认模型"}>
                        {item.defaultModel || item.baseUrl || "未设置默认模型"}
                    </div>
                </div>
