...
本文档用于约束本项目中的 AI / 自动化开发行为。用户当前明确要求优先于本文件。

## 定制开发入口

- 开始定制前先读 [定制开发规则](docs/customizations/rules.md) 和 [修改记录](docs/customizations/changes.md)。规则集中维护在前者，每项变更按后者关联独立记录。
- 下文后端、前端和画布规范中的目录约定适用于上游业务维护；定制实现、配置、样式和数据按定制开发规则集中隔离。复用上游技术栈和行为规范，不把定制实现散入上游目录。
...
- 上游数据结构维护按实际需求处理；定制数据的存储、迁移和回退统一遵循定制开发规则，不以项目尚未上线为由直接侵入上游表结构。
- 按变更风险执行必要验证；文档变更检查链接、规则一致性和 Git 差异，业务变更检查相应测试或运行行为，未验证部分明确说明。
- 不要改无关文件，不要顺手重构。
- 在用户已授权范围内直接执行必要修改；新增范围或未授权的破坏性操作才另行确认。
